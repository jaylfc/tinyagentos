...
- External binding: set `gateway.bindHost: "0.0.0.0"` or equivalent (unknown exact config key — needs primary source verification; see §5.4)
...
The `api: "openai-completions"` mode uses the `/v1/chat/completions` endpoint. Tool calling reliability in this mode depends on the upstream model. Since taOS's LiteLLM proxy handles routing to real models, reliability is governed by the backend model, not openclaw's compat layer.
...
### 5.4 `gateway.bindHost` config key

The exact config key for the gateway bind address is not confirmed from docs (the relevant page 404'd). The README quick start uses `openclaw gateway --port 18789 --verbose` CLI flags. Config schema verification via `config.schema` RPC at runtime is the reliable method.

**Open question — needs primary source:** exact `openclaw.json` key to change bind address from loopback to all interfaces.
...
# 7. Wait for gateway to start (use ss/netstat as fallback; 'openclaw gateway status' not verified)
...
1. **Exact bind-host config key** in `openclaw.json` for changing bind address from loopback to all interfaces. Docs page 404'd.
...
5. **`openclaw gateway status` CLI command** — exact syntax for health-check in install.sh startup wait loop.

6. **MCP server registration in plugins** — exact `api.register*` call for MCP server integration.
...
The following URLs returned 404 or were otherwise unavailable at research time:

- `docs.openclaw.ai/concepts/configuration` — 404; used `docs.openclaw.ai/configuration` instead
- `docs.openclaw.ai/concepts/plugins` — 404; used `docs.openclaw.ai/plugins` instead
- `docs.openclaw.ai/concepts/sessions` — 404
- `docs.openclaw.ai/gateway/logs` — 404
- `docs.openclaw.ai/operations/logging` — 404
- `docs.openclaw.ai/remote-access` — 404
- `docs.openclaw.ai/deploy/docker` — 404
- `docs.openclaw.ai/deploy/linux` — 404
- `docs.openclaw.ai/deploy/raspberry-pi` — 404
- `docs.openclaw.ai/gateway/configuration-reference` — fetched but limited detail on provider schema
- `docs.openclaw.ai/providers/lm-studio` — 404
- `docs.openclaw.ai/api/sessions` — 404
