#!/bin/bash
set -euo pipefail
log() { echo "[$(date -u +%H:%M:%S)] pocketflow-install: $*"; }
...
log "pip install pocketflow + deps"
pip3 install --break-system-packages --quiet pocketflow openai httpx 2>&1 | tail -3
mkdir -p /opt/taos
cat > /opt/taos/taos-pocketflow-bridge.py <<'BRIDGE_EOF'
#!/usr/bin/env python3
"""taOS-PocketFlow bridge."""
from __future__ import annotations
import asyncio, json, logging, os, signal
from concurrent.futures import ThreadPoolExecutor
import httpx
logging.basicConfig(level=logging.INFO, format="%(asctime)s [pf-bridge] %(message)s")
log = logging.getLogger("pf-bridge")
...
asyncio.run(main())
BRIDGE_EOF
chmod +x /opt/taos/taos-pocketflow-bridge.py
cat > /etc/systemd/system/taos-pocketflow-bridge.service <<UNIT
[Unit]
Description=taOS-pocketflow bridge
...
ExecStart=/usr/bin/python3 /opt/taos/taos-pocketflow-bridge.py
Restart=on-failure
RestartSec=5
StandardOutput=append:/var/log/taos-pocketflow-bridge.log
StandardError=append:/var/log/taos-pocketflow-bridge.log
[Install]
